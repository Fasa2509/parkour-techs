...
export const ConfirmationEmail = ({ userName, token }: { userName: string; token: string; }) => {
    return (
        <Html lang="es">
            <Container>
                <Text style={{ fontSize: '28px', color: '#777', fontWeight: 'bold', textAlign: 'center' }}>Parkour Techs</Text>
                <Text>¡Hola {userName} y bienvenidos a Parkour Techs!</Text>
                ...
                <Section style={{ backgroundColor: '#ccdddd', borderRadius: '12px' }}>
                    <Text style={{ paddingLeft: '12px' }}>Para activar tu cuenta, pulsa <Link href={DOMAIN_NAME + '/confirm?token=' + token}>aquí</Link>.</Text>
                </Section>
            </Container>
        </Html>
    )
